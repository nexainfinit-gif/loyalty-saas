...
import { PageStyles, TopBand, NeutralLoader, type PubEvent, type PubBusiness } from '../../../event/[slug]/shared'

/**
 * Page événement — version POLIE (parallèle, ne remplace pas l'existante).
 * Mêmes 4 thèmes, même logique métier (API, achat Stripe, parrainage).
 * Poli : hiérarchie & rythme, UX du formulaire, retrait des fioritures ✦/⚡.
 */
function EventDetailContent() {
  ...
  // Pastille d'urgence propre (sans emoji) — un point coloré + le compte.
  const urgencyPill = (count: number) => (
    <span className="inline-flex items-center gap-1.5 ev-mono text-[10px] font-bold uppercase tracking-[0.15em]" style={{ color: C.accent2 }}>
      <span className="inline-block w-1.5 h-1.5 rounded-full" style={{ background: C.accent2 }} />
      {t('event.fewLeft', { count })}
    </span>
  )

  // Ligne d'info (date / lieu / prix) — plus lisible que des chips serrées.
  const infoRow = (label: string, value: string) => (
    <div className="flex items-baseline gap-3 py-2.5" style={{ borderTop: `1px solid ${C.border}` }}>
      <span className="ev-mono text-[9px] uppercase tracking-[0.25em] shrink-0 w-14" style={{ color: C.faint }}>{label}</span>
  ...
  return (
    <Shell>
      <div className="max-w-lg mx-auto px-4 pb-28 sm:pb-16">
        ...
        <header className="pt-7 pb-7 ev-up" style={{ animationDelay: '0.05s' }}>
          <p className="ev-mono text-[10px] uppercase tracking-[0.32em] mb-4" style={{ color: C.accent }}>Rebites Events</p>
          ...
          {/* Méta : date / lieu / prix en lignes nettes */}
          <div className="mt-6">
            {infoRow(t('event.dateLabel'), `${longDate} · ${time}`)}
            {ev.location && infoRow(t('event.locationLabel'), ev.location)}
            <div className="flex items-center justify-between py-2.5" style={{ borderTop: `1px solid ${C.border}`, borderBottom: `1px solid ${C.border}` }}>
              <span className="ev-mono text-[9px] uppercase tracking-[0.25em] shrink-0 w-14" style={{ color: C.faint }}>{t('event.priceLabel')}</span>
              <span className="ev-mono text-sm font-bold" style={{ color: C.ink }}>{priceBadge}</span>
            </div>
          </div>

          {ev.remaining !== null && !soldOut && ev.remaining <= 5 && (
            <div className="mt-4">{urgencyPill(ev.remaining)}</div>
          )}
        </header>
            ...
            <form onSubmit={submit} className="space-y-4">
                  ...
                  {ev.tiers.map(tr => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full text-left px-3.5 py-3 transition-colors disabled:opacity-40"
                        style={{
                      ...
                      >
                        <span className="flex items-center justify-between gap-3">
                          <span className="min-w-0">
                            ...
                            {trSoldOut ? (
                              <span className="ev-mono block text-[10px] uppercase tracking-[0.15em] mt-1 line-through" style={{ color: C.faint }}>
                                {t('event.soldOut')}
                              </span>
                            ) : tr.remaining !== null && tr.remaining <= 5 && (
                              <span className="block mt-1.5">{urgencyPill(tr.remaining)}</span>
                            )}
                          </span>
              ...
              <div className="space-y-2.5">
                <input required value={buyerName} onChange={e => setBuyerName(e.target.value)} maxLength={100}
                  autoComplete="name"
                  placeholder={t('event.buyerName')} className={`ev-input-${C.key} w-full px-3.5 py-3 text-base sm:text-sm`} />
                <input required type="email" value={buyerEmail} onChange={e => setBuyerEmail(e.target.value)} maxLength={255}
                  autoComplete="email" inputMode="email"
                  placeholder={t('event.buyerEmail')} className={`ev-input-${C.key} w-full px-3.5 py-3 text-base sm:text-sm`} />
              </div>

              <div className="space-y-2 pt-0.5">
                {ev.offer_loyalty && (
                  <label className="flex items-start gap-2.5 text-xs cursor-pointer" style={{ color: C.muted }}>
                    ...
                    <span className="leading-relaxed">{t('event.joinLoyalty', { business: business.name })}</span>
                  </label>
                )}
                <label className="flex items-start gap-2.5 text-xs cursor-pointer" style={{ color: C.muted }}>
                  <input type="checkbox" checked={marketingOptIn} onChange={e => setMarketingOptIn(e.target.checked)}
                    className="mt-0.5" style={{ accentColor: C.accent }} />
                  <span className="leading-relaxed">{t('event.marketingOptIn', { business: business.name })}</span>
                </label>
              </div>
