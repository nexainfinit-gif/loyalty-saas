 ...
 * Présentation pilotée par le THÈME choisi par l'organisateur :
 * « nuit » (poster-brutalisme concert), « corporate » (clair raffiné),
 * « musée » (blanc galerie). Tokens dans lib/event-themes.ts.
 */
interface PubEvent {
  id: string
  title: string
...
function ThemeStyles({ T }: { T: EventTheme }) {
  return (
    <style>{`
      ...
      .ev-bg { background: ${T.bg}; position: relative; overflow-x: hidden; }
      ${T.grain ? `
      .ev-bg::before {
      ...
      .ev-btn:active:not(:disabled) { ${T.dark ? 'transform: translate(2px, 2px); box-shadow: 1px 1px 0 #000;' : 'transform: translateY(1px);'} }
      .ev-input {
        background: ${T.dark ? T.bg : T.surface};
        border: ${T.dark ? '2px' : '1px'} solid ${T.dark ? '#2e2e38' : T.border};
        color: ${T.ink}; border-radius: ${T.radius};
      }
      .ev-input:focus { outline: none; border-color: ${T.accent}; ${T.dark ? 'box-shadow: 4px 4px 0 rgba(200,255,46,0.25);' : `box-shadow: 0 0 0 3px ${T.accent}22;`} }
      .ev-input::placeholder { color: ${T.faint}; }
      .ev-stub-code {
        background: ${T.dark ? '#F4F4F6' : T.headerBg};
        color: ${T.dark ? '#0B0B10' : '#FFFFFF'};
        border-radius: ${T.radius};
        ${T.dark ? `border: 2px solid #F4F4F6; box-shadow: 5px 5px 0 ${T.accent2};` : `box-shadow: ${T.shadow};`}
        transition: all 0.15s ease; display: block;
      }
      .ev-stub-code:hover { ${T.dark ? `transform: translate(-2px, -2px); box-shadow: 7px 7px 0 ${T.accent};` : `box-shadow: ${T.shadowHover}; transform: translateY(-2px);`} }
    `}</style>
  )
}

/** Bandeau marquee (thème nuit) ou filet éditorial (thèmes clairs). */
...
function EventContent() {
      ...
      <Shell>
        <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
          <div className="w-full max-w-md ev-up">
            ...
            <h1 className="ev-display text-3xl leading-tight mb-2" style={{ color: T.ink }}>{t('event.successDesc')}</h1>
      ...
      <Shell>
        <div className="min-h-[80vh] flex items-center justify-center px-4">
          <div className="max-w-sm ev-up">
            <h1 className="ev-display text-2xl mb-3" style={{ color: T.ink }}>{t('event.paymentIssue')}</h1>
  ...
  const accent = business.primaryColor && business.primaryColor !== '#ffffff' ? business.primaryColor : T.accent2
    ...
    <Shell>
      <div className="max-w-lg mx-auto px-4 pb-16">
        ...
        <header className="pt-10 pb-8 ev-up">
          ...
          <h1 className="ev-display text-4xl sm:text-5xl leading-[1.02] break-words" style={{ color: T.ink }}>
            {business.name}
          </h1>
        ...
        <div className="space-y-6">
          {events.map((ev, idx) => {
            ...
            return (
              <article key={ev.id} className="ev-card ev-up overflow-hidden" style={{ animationDelay: `${0.08 + idx * 0.07}s` }}>
                <div className="flex">
                  {/* Bloc date — talon gauche */}
                  <div className="flex flex-col items-center justify-center px-4 py-5 border-r border-dashed min-w-[86px]" style={{ borderColor: T.border }}>
                    <span className="ev-display text-3xl leading-none" style={{ color: T.accent }}>{day}</span>
                    <span className="ev-mono text-[11px] uppercase tracking-[0.2em] mt-1" style={{ color: T.muted }}>{month}</span>
                    <span className="ev-mono text-[11px] mt-2" style={{ color: T.faint }}>{time}</span>
                  </div>

                  <div className="flex-1 p-5 min-w-0">
                    <div className="flex items-start justify-between gap-3">
                      <h2 className="ev-display text-xl leading-tight break-words" style={{ color: T.ink }}>{ev.title}</h2>
                      <span
                    ...
                    {ev.description && (
                      <p className="text-sm mt-2 whitespace-pre-line leading-relaxed" style={{ color: T.muted }}>{ev.description}</p>
                    )}
                    {ev.remaining !== null && !soldOut && ev.remaining <= 5 && (
                      <p className="ev-mono text-[11px] font-bold uppercase tracking-[0.15em] mt-2" style={{ color: T.accent2 }}>
                    ...
                    {soldOut ? (
                      ...
                      <form onSubmit={e => submit(e, ev)} className="mt-4 space-y-3">
                        <div className="flex items-center gap-3">
                          <label className="ev-mono text-[11px] uppercase tracking-[0.15em]" style={{ color: T.muted }}>{t('event.quantity')}</label>
                            ...
                            {[1, 2, 3, 4, 5, 6]
                              ...
                              .map(n => (
                                <button
                                  ...
                                  style={quantity === n
                                    ? { background: T.accent, color: T.accentInk, border: `2px solid ${T.accent}`, borderRadius: T.radius }
                                    : { color: T.muted, border: `2px solid ${T.border}`, borderRadius: T.radius }}
                                >
                                  {n}
                                </button>
                              ))}
                        ...
                        {ev.offer_loyalty && (
                          <label className="flex items-start gap-2 text-xs" style={{ color: T.muted }}>
                            <input type="checkbox" checked={joinLoyalty} onChange={e => setJoinLoyalty(e.target.checked)}
                              className="mt-0.5" style={{ accentColor: T.accent }} />
                            {t('event.joinLoyalty', { business: business.name })}
                          </label>
                        )}
                        {error && (
                          <p className="ev-mono text-xs px-3 py-2" style={{ color: T.accent2, border: `2px solid ${T.accent2}`, borderRadius: T.radius }}>{error}</p>
                        )}
                        <button
                        ...
                        {ev.price > 0 && (
                          <p className="ev-mono text-[10px] uppercase tracking-[0.2em] text-center" style={{ color: T.faint }}>{t('event.securePayment')}</p>
                        )}
                      </form>
                    )}
                  </div>
