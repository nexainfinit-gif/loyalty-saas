...
function CreatePlanModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.key.trim() || !form.name.trim()) {
      setError(t('admin.plansKeyRequired'));
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/20 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl border border-gray-100 shadow-xl w-full max-w-md p-6">
        <h2 className="text-base font-semibold text-gray-900 mb-5">{t('admin.plansCreateTitle')}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">{t('admin.plansKeyLabel')}</label>
            <input
              value={form.key}
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">{t('admin.plansNameLabel')}</label>
            <input
              value={form.name}
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">{t('admin.plansPriceLabel')}</label>
            <input
              type="number"
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              className="rounded"
            />
            <label htmlFor="is_public" className="text-sm text-gray-700">{t('admin.plansPublicLabel')}</label>
          </div>
      ...
      <header className="bg-white border-b border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold text-gray-900">{t('admin.plansTitle')}</h1>
          </div>
          <button
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100">
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wide">{t('admin.plansHeaderPlan')}</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wide">{t('admin.plansHeaderKey')}</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wide">{t('admin.plansHeaderPrice')}</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wide">{t('admin.plansHeaderPublic')}</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wide">{t('admin.plansHeaderActive')}</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wide">{t('admin.plansHeaderFeatures')}</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wide">{t('common.actions')}</th>
              </tr>
            </thead>
              ...
              {plans.map((plan) => (
                <tr key={plan.id} className={`hover:bg-gray-50/50 transition-colors ${!plan.is_active ? 'opacity-50' : ''}`}>
                  ...
                  <td className="px-4 py-3">
                    {plan.is_public
                      ? <span className="text-emerald-600 text-xs font-medium">{t('common.yes')}</span>
                  ...
                  <td className="px-4 py-3">
                    {plan.is_active
                      ...
                      : <span className="text-gray-400 text-xs">{t('admin.statusArchived')}</span>}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      >
                        {plan.is_active ? t('admin.plansArchive') : t('admin.plansActivate')}
                      </button>
                    </div>
                  </td>
                </tr>
