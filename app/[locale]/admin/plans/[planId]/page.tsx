...
export default function AdminPlanEditPage() {
  ...
  async function saveMeta(e: React.FormEvent) {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) { setMetaMsg(json.error ?? t('common.error')); return; }
      setPlan((p) => p ? { ...p, ...json.plan } : p);
      setMetaMsg(t('admin.planDetailSaved'));
    } finally {
      setSavingMeta(false);
    }
  }
  ...
  async function saveFeatures() {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) { setFeatMsg(json.error ?? t('common.error')); return; }
      setFeatures(json.features ?? features);
      setFeatMsg(t('admin.planDetailFeaturesSaved'));
    } finally {
    ...
    return (
      <div className="min-h-screen bg-surface flex items-center justify-center">
        <div className="bg-white rounded-2xl border border-red-100 p-8 text-center">
          ...
          <button onClick={() => router.back()} className="mt-4 text-sm text-primary-600 hover:underline">{t('admin.detailBack')}</button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-surface">
      ...
      <main className="max-w-3xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-6">
          <h2 className="text-sm font-semibold text-gray-700 mb-5">{t('admin.planDetailInfoTitle')}</h2>
          <form onSubmit={saveMeta} className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">{t('admin.planDetailNameLabel')}</label>
              <input
                value={meta.name}
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">{t('admin.planDetailKeyLabel')}</label>
              <input
                value={plan.key}
                readOnly
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">{t('admin.planDetailPriceLabel')}</label>
              <input
                type="number"
                value={meta.price_monthly}
                onChange={(e) => setMeta({ ...meta, price_monthly: e.target.value })}
                placeholder={t('admin.planDetailPricePlaceholder')}
                className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary-600/20"
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">{t('admin.planDetailOrderLabel')}</label>
              <input
                type="number"
            ...
            <div className="flex gap-6">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="rounded"
                />
                <span className="text-sm text-gray-700">{t('admin.planDetailPublicLabel')}</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  ...
                  className="rounded"
                />
                <span className="text-sm text-gray-700">{t('admin.planDetailActiveLabel')}</span>
              </label>
            </div>
            <div className="flex items-center gap-4 pt-2">
              ...
              >
                {savingMeta ? t('common.savingDots') : t('common.save')}
              </button>
              {metaMsg && (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-6">
          <h2 className="text-sm font-semibold text-gray-700 mb-5">{t('admin.planDetailFeaturesTitle')}</h2>
          ...
          <div className="mt-4 pt-4 border-t border-gray-50 flex gap-2">
            <input
              ...
              placeholder={t('admin.planDetailAddFeature')}
          ...
          <div className="flex items-center gap-4 mt-5 pt-4 border-t border-gray-100">
            <button
              ...
              className="px-5 py-2 rounded-xl text-sm font-semibold bg-primary-600 text-white hover:bg-primary-700 disabled:opacity-50 transition-colors"
            >
              {savingFeatures ? t('common.savingDots') : t('admin.planDetailSaveFeatures')}
            </button>
            {featMsg && (
