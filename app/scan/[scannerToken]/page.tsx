...
// Share the URL with cashiers; rotate it from the dashboard to revoke access.
...
export default function PublicScannerPage() {
  ...
  // ── Validate scanner token on mount ─────────────────────────────────────
  useEffect(() => {
  ...
  async function startCamera() {
    ...
    try {
      ...
      const msg =
        err.name === 'NotAllowedError'  ? 'Accès caméra refusé. Autorisez-la dans les réglages.' :
  ...
  async function processScan(token: string) {
    ...
    try {
      const res = await fetch(`/api/scan/${encodeURIComponent(token)}`, {
        method: 'POST',
        // Auth: scanner_token in custom header — no Supabase session needed
        headers: { 'X-Scanner-Token': scannerToken },
      });
      const data = await res.json();
      if (!res.ok) { setErrorMsg(data.error || 'Erreur lors du scan'); setStatus('error'); return; }
      setResult(data);
      setStatus('success');
      setManualId('');
    } catch {
  ...
  function reset() { setResult(null); setStatus('idle'); setErrorMsg(''); setManualId(''); }
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#F8F9FA', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', fontFamily: "'DM Sans', sans-serif" }}>
        <div style={{ textAlign: 'center', maxWidth: '360px' }}>
          ...
          <h1 style={{ fontSize: '1.25rem', fontWeight: 700, color: '#111827', margin: '0 0 0.5rem' }}>Lien invalide</h1>
          <p style={{ color: '#6B7280', fontSize: '0.9rem' }}>Ce lien scanner est invalide ou a été révoqué. Demandez un nouveau lien au propriétaire.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8F9FA', fontFamily: "'DM Sans', sans-serif", padding: '1.5rem', maxWidth: '480px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '2rem', paddingBottom: '1.25rem', borderBottom: '1px solid #F3F4F6' }}>
        ...
        <h1 style={{ margin: 0, fontSize: '1.3rem', fontWeight: 700, color: '#111827' }}>Scanner QR Code</h1>
        ...
        <div className="result-card" style={{ background: 'white', borderRadius: '20px', padding: '2rem', textAlign: 'center', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', marginBottom: '1.5rem', position: 'relative', overflow: 'hidden' }}>
          ...
          {result.stamp_card_completed ? (
            <>
              ...
              <h2 style={{ color: '#059669', fontWeight: 700, margin: '0 0 0.5rem' }}>Carte terminée !</h2>
            ...
            <>
              <div style={{ fontSize: '3rem', marginBottom: '0.75rem' }}>🏆</div>
              <h2 style={{ color: '#D97706', fontWeight: 700, margin: '0 0 0.5rem' }}>Récompense débloquée !</h2>
            ...
            {result.program_type === 'stamps' ? (
              <>
                <div style={{ background: result.stamp_card_completed ? '#D1FAE5' : '#F0FDF4', borderRadius: '12px', padding: '0.875rem 1.5rem' }}>
                  <p style={{ margin: 0, fontSize: '0.75rem', color: '#6B7280' }}>{result.stamp_card_completed ? 'Carte complétée' : 'Tampon ajouté'}</p>
                ...
                <div style={{ background: '#EFF6FF', borderRadius: '12px', padding: '0.875rem 1.5rem' }}>
                  <p style={{ margin: 0, fontSize: '0.75rem', color: '#6B7280' }}>{result.stamp_card_completed ? 'Nouvelle carte' : 'Carte'}</p>
      ...
      {status !== 'success' && (
        <>
          ...
          <div style={{ background: 'white', borderRadius: '20px', padding: '1.5rem', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', border: '1.5px solid #F3F4F6' }}>
            <p style={{ fontWeight: 600, margin: '0 0 0.25rem', fontSize: '0.9rem', color: '#374151' }}>Ou saisir le code manuellement</p>
            <p style={{ margin: '0 0 0.75rem', fontSize: '0.75rem', color: '#9CA3AF' }}>Copiez le code affiché sous le QR du pass client</p>
            <input
              value={manualId}
