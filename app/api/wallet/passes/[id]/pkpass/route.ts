...
export async function GET(
  ...
  const { data: restaurant, error: restErr } = await supabaseAdmin
    .from('restaurants')
    .select('id, name, slug, business_type, primary_color, logo_url, booking_active')
  ...
  const input: PassBuildInput = {
    ...
    bookingUrl:          isBookingOpen(restaurant as { booking_active?: boolean | null })
      ...
      : null,
    portalUrl:           `${process.env.NEXT_PUBLIC_APP_URL ?? ''}/fr/client/${(restaurant as { slug?: string }).slug}?t=${customer.qr_token ?? ''}`,
  };
