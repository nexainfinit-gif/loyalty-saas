...
const settingsSchema = z.object({
  ...
  no_show_block_threshold:     z.number().int().min(0).max(10),
  deposit_enabled:             z.boolean().optional().default(false),
  deposit_type:                z.enum(['fixed', 'percent']).optional().default('fixed'),
  deposit_value:               z.number().min(0).max(1000).optional().default(10),
