...
export async function GET(req: NextRequest) {
  ...
  if (customer.email_verified) {
    ...
    return new NextResponse(successHtml(customer.first_name, true, walletUrls), {
      status: 200,
      headers: { 'Content-Type': 'text/html; charset=utf-8' },
    });
  }
  ...
  return new NextResponse(successHtml(customer.first_name, false, walletUrls), {
    status: 200,
    headers: { 'Content-Type': 'text/html; charset=utf-8' },
  });
}
...
function successHtml(firstName: string, alreadyVerified: boolean, walletUrls: WalletUrls): string {
  ...
  const walletSection = hasWallet ? `
    <div style="background: #f8f9fa; border-radius: 16px; padding: 1.25rem; margin-top: 1.5rem; border: 1.5px solid #e5e7eb;">
      ...
      ${walletUrls.apple ? `
      <a href="${walletUrls.apple}" style="display: flex; align-items: center; justify-content: center; gap: 0.5rem; background: #000; color: white; text-decoration: none; padding: 0.875rem; border-radius: 12px; font-size: 0.9rem; font-weight: 600; margin-bottom: ${walletUrls.google ? '0.75rem' : '0'};">
        <svg width="18" height="22" viewBox="0 0 20 24" fill="white"><path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8 1.18-.24 2.31-.93 3.57-.84 1.51.12 2.65.72 3.4 1.8-3.12 1.87-2.38 5.98.48 7.13-.57 1.5-1.31 2.99-2.54 4.09zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.29 2.58-2.34 4.5-3.74 4.25z"/></svg>
        Ajouter à Apple Wallet
      </a>` : ''}

      ${walletUrls.google ? `
      <a href="${walletUrls.google}" target="_blank" rel="noreferrer" style="display: flex; align-items: center; justify-content: center; gap: 0.5rem; background: #1a73e8; color: white; text-decoration: none; padding: 0.875rem; border-radius: 12px; font-size: 0.9rem; font-weight: 600;">
