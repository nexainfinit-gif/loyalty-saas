...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!consent_marketing) {
      setErrorMsg('Vous devez accepter de recevoir vos récompenses.');
      setStatus('error');
      return;
    }
    ...
    if (!res.ok) {
      setErrorMsg(
        res.status === 409
          ? 'Cet email est déjà inscrit à ce programme.'
  ...
  if (loadingResto) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#F8F9FA' }}>
      <p style={{ color: '#9CA3AF', fontFamily: 'DM Sans, sans-serif' }}>⏳ Chargement...</p>
    </div>
  );
  ...
  if (notFound) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#F8F9FA', fontFamily: 'DM Sans, sans-serif' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <h2 style={{ fontWeight: 700, margin: '0 0 0.5rem' }}>Commerce introuvable</h2>
        <p style={{ color: '#9CA3AF' }}>Le lien que vous avez utilisé est invalide.</p>
      </div>
    </div>
  );
        ...
        {step === 'form' && (
          <div style={{ padding: '1.75rem' }}>
            <p style={{ textAlign: 'center', color: '#6B7280', fontSize: '0.875rem', margin: '0 0 1.5rem', lineHeight: 1.6 }}>
              Rejoignez le programme fidélité de <strong style={{ color: '#111827' }}>{restaurant?.name}</strong> et recevez vos récompenses directement sur votre téléphone.
            </p>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div className="field">
                ...
                <input name="first_name" type="text" placeholder="Votre prénom" required />
              </div>
              ...
              <div className="field">
                <label style={{ fontSize: '0.75rem', fontWeight: 600, color: '#374151', display: 'block', marginBottom: '0.4rem' }}>
                  Date de naissance
                  <span style={{ color: '#9CA3AF', fontWeight: 400, marginLeft: '0.25rem' }}>(pour un cadeau d'anniversaire 🎂)</span>
                </label>
              ...
              <div className="field">
                <label style={{ fontSize: '0.75rem', fontWeight: 600, color: '#374151', display: 'block', marginBottom: '0.4rem' }}>
                  Téléphone
                  <span style={{ color: '#9CA3AF', fontWeight: 400, marginLeft: '0.25rem' }}>(optionnel)</span>
                </label>
              ...
              <label style={{
                ...
                <span style={{ fontSize: '0.78rem', color: '#374151', lineHeight: 1.6 }}>
                  J'accepte de recevoir mes récompenses et offres personnalisées de <strong>{restaurant?.name}</strong> par email. *
                </span>
              </label>
              ...
              >
                {status === 'loading' ? '⏳ Inscription...' : '🎁 Recevoir ma carte fidélité'}
              </button>

              <p style={{ textAlign: 'center', fontSize: '0.72rem', color: '#9CA3AF', margin: 0 }}>
                Données protégées · Aucun spam · Désabonnement gratuit
              </p>
            </form>
          </div>
        )}
          ...
          <div style={{ padding: '2rem', textAlign: 'center' }}>
            ...
            <div style={{
              ...
              <p style={{ fontSize: '0.82rem', color: '#374151', margin: 0, lineHeight: 1.6 }}>
                🎁 <strong>10 points de bienvenue</strong> ont été ajoutés à votre carte !
              </p>
            </div>
          </div>
        )}
